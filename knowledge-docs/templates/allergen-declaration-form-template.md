# Allergen Declaration Form Template

## Supplier Allergen Declaration

**Supplier Name:** _______________  
**Product Name:** _______________  
**Product Code:** _______________  
**Batch/Lot Number:** _______________  
**Date of Declaration:** _______________

---

## Declaration

This form must be completed for all food products supplied. Please indicate which of the 14 major allergens are present in this product as **intentional ingredients** or through **cross-contamination risk**.

---

## 14 Major Allergens (EU/UK Law)

| Allergen | Intentionally Added | Present Due to Cross-Contamination Risk | Not Present |
|----------|---------------------|----------------------------------------|-------------|
| **1. Cereals containing gluten** (wheat, rye, barley, oats, spelt, kamut) | [ ] | [ ] | [ ] |
| If yes, specify which: _______________ | | | |
| **2. Crustaceans** (prawns, crabs, lobster, crayfish) | [ ] | [ ] | [ ] |
| **3. Eggs** | [ ] | [ ] | [ ] |
| **4. Fish** | [ ] | [ ] | [ ] |
| **5. Peanuts** | [ ] | [ ] | [ ] |
| **6. Soybeans** | [ ] | [ ] | [ ] |
| **7. Milk** (including lactose) | [ ] | [ ] | [ ] |
| **8. Nuts** (almonds, hazelnuts, walnuts, cashews, pecans, Brazil nuts, pistachios, macadamia nuts) | [ ] | [ ] | [ ] |
| If yes, specify which: _______________ | | | |
| **9. Celery** (including celeriac) | [ ] | [ ] | [ ] |
| **10. Mustard** | [ ] | [ ] | [ ] |
| **11. Sesame seeds** | [ ] | [ ] | [ ] |
| **12. Sulphur dioxide and sulphites** (>10mg/kg or 10mg/litre) | [ ] | [ ] | [ ] |
| **13. Lupin** | [ ] | [ ] | [ ] |
| **14. Molluscs** (mussels, oysters, squid, snails) | [ ] | [ ] | [ ] |

---

## Cross-Contamination Controls

**If allergens are present due to cross-contamination risk, describe controls in place to minimize risk:**

_______________

**Are allergen-containing and allergen-free products produced on the same line?**

- [ ] Yes
- [ ] No

**If yes, describe cleaning procedures between runs:**

_______________

---

## Additional Allergens (Non-Mandatory but Important)

| Allergen | Present | Not Present | Comments |
|----------|---------|-------------|----------|
| **Kiwi** | [ ] | [ ] | |
| **Buckwheat** | [ ] | [ ] | |
| **Pine nuts** | [ ] | [ ] | |
| **Chickpeas** | [ ] | [ ] | |
| **Poppy seeds** | [ ] | [ ] | |

---

## Precautionary Allergen Labelling (PAL)

**Does the product label include a "may contain" statement?**

- [ ] Yes
- [ ] No

**If yes, please state:** _______________

**Is the "may contain" statement justified by risk assessment?**

- [ ] Yes
- [ ] No

**Risk assessment reference/date:** _______________

---

## Supplier Confirmation

I confirm that the information provided in this declaration is accurate and complete to the best of my knowledge. I will notify the customer immediately of any changes to ingredients, processes, or allergen status.

**Completed by (Name):** _______________  
**Job Title:** _______________  
**Company:** _______________  
**Signature:** _______________  
**Date:** _______________

---

## Customer Use Only

**Declaration received by:** _______________  
**Date received:** _______________  
**Product specification updated:** [ ] Yes [ ] No  
**HACCP plan reviewed:** [ ] Yes [ ] No  
**Labelling updated (if applicable):** [ ] Yes [ ] No

---

**Record Retention:** Duration of supplier relationship + 3 years

---

## Notes for Suppliers

1. **Intentionally added allergens:** Allergens used as ingredients in the product formulation.
2. **Cross-contamination risk:** Allergens not in the product but present in the same facility, on shared equipment, or in raw materials from suppliers.
3. **"May contain" statements:** Use only where genuine cross-contamination risk exists and cannot be eliminated through cleaning or segregation. Avoid blanket "may contain" statements not supported by risk assessment.
4. **Notification of changes:** You must inform the customer **in advance** of any changes to ingredients, suppliers, or manufacturing processes that could affect allergen status.
5. **Traceability:** Ensure full traceability of allergen-containing ingredients (supplier batch codes, delivery records).
