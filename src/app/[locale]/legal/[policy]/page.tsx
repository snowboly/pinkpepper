...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; policy: string }> }): Promise<Metadata> { const { locale, policy } = await params; if (!isLegalLocale(locale) || locale === "en" || !isPolicy(policy)) return {}; const document = getLegalDocument(policy, locale); return { title: document.title + " | PinkPepper", description: document.description, alternates: { canonical: "https://pinkpepper.io/" + locale + "/legal/" + policy, languages: getLegalAlternates(policy) }, robots: { index: false, follow: true } }; }
