...
export async function renderCleaningSopPdf(data: CleaningSopData): Promise<Uint8Array> {
  ...
  function drawTable(headers: string[], rows: string[][], colWidths?: number[]) {
    ...
    rows.forEach((row) => {
      ensureSpace(rowH);
      xPos = MARGIN;
      page.drawRectangle({ x: MARGIN, y: y - rowH + 4, width: tableWidth, height: rowH, borderColor: BORDER, borderWidth: 0.5 });
      row.forEach((value, i) => {
  ...
  const respW = [160, CONTENT_W - 160];
  drawTable(["Role", "Responsibility"], data.responsibilities.map((r) => [r.role, r.responsibility]), respW);
  ...
  const freqW = [160, 160, 110, CONTENT_W - 430];
  drawTable(
  ...
  const atpW = [180, CONTENT_W / 4, CONTENT_W / 4, CONTENT_W - 180 - CONTENT_W / 2];
  drawTable(
